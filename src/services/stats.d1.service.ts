// Service Stats adapté pour D1 (async)
...
/**
 * Récupère les stats d'une semaine
 */
export async function getWeeklyStats(db: D1Database, weekStr?: string): Promise<PeriodStats> {
  const now = new Date();
  if (!weekStr) {
  ...
  return getPeriodStats(db, startDate, endDate, weekStr);
}

/**
 * Récupère les stats d'un mois
 */
export async function getMonthlyStats(db: D1Database, monthStr?: string): Promise<PeriodStats> {
  const now = new Date();
  if (!monthStr) {
    const year = now.getFullYear();
  ...
  return getPeriodStats(db, startDate, endDate, monthStr);
}
...
async function getPeriodStats(
  ...
  `).bind(startDate, endDate).first<{ total_xp: number; days_with_entries: number }>();
  ...
  const daysWithEntries = totalResult?.days_with_entries ?? 0;
  
  return {
    period,
    startDate,
    endDate,
    totalXp: totalResult?.total_xp ?? 0,
    byCategory: (categoryResults.results || []).map(r => ({
    ...
    dailyAverage: daysWithEntries > 0 
      ? Math.round((totalResult?.total_xp ?? 0) / daysWithEntries) 
      : 0,
    daysWithEntries
  };
}
...
export async function getDailyHistory(
  ...
  `).bind(days).all<{ date: string; total_xp: number }>();
